...
def rand_weights(seed=43, sparsity=0.4, cv=1.0, dt=0.1, horizon=256, num_node=90):
    ...
    Args:
        seed (int): Seed for random number generation. Default is 43.
        sparsity (float): Sparsity of the weights. Default is 0.4.
        cv (float): Coefficient of variation. Default is 1.0.
        dt (float): Time step. Default is 0.1.
        horizon (int): Horizon length. Default is 256.
        num_node (int): Number of nodes. Default is 90.

    Returns:
        tuple: A tuple containing the following elements:
            - weights (ndarray): Randomly generated weights.
            - lengths (ndarray): Lengths of the weights.
            - cx_j mode (tuple): A tuple containing the following elements:
                - data (ndarray): Data of the sparse weights.
                - indices (ndarray): Indices of the sparse weights.
                - indptr (ndarray): Index pointers of the sparse weights.
                - idelays (ndarray): Delays of the sparse weights.
    """
    ...
    idelays = (lengths.T.copy().ravel()[nnz_ravel] / cv / dt).astype("i") + 2
    return (
        weights,
        lengths,
        (
            spw.data.astype(np.float32),
            spw.indices.astype(np.uint32),
            spw.indptr.astype(np.uint32),
            idelays,
        ),
    )
...
def base_setup():  # mode: tvb_kernels.CxMode = tvb_kernels.CxMode.CX_J):
    ...
    """
    Set up the base configuration for testing.

    Returns:
        tuple: A tuple containing the following elements:
            - conn (m.Conn): Connection object.
            - cx (m.Cx): Cx object.
            - make_cfun_np (function): Function that generates a numpy version of cfun.
    ...
    num_node = 90
    horizon = 256
    weights, lengths, spw = rand_weights(
        num_node=num_node, horizon=horizon, dt=dt, cv=cv
    )
    ...
    # then we can test
    buf_val = (
        np.r_[: 1.0 : 1j * num_node * horizon].reshape(num_node, horizon).astype("f")
        * 4.0
    )
    cx.buf[:] = buf_val
...
def base_setup_simd():
    ...
    # then we can test
    buf_val = (
        np.r_[: 1.0 : 1j * num_node * horizon * 8]
        .reshape(num_node, horizon, 8)
        .astype("f")
        * 4.0
    )
    cx.buf[:] = buf_val
...
def base_setup_simd_batch():
    ...
    # then we can test
    buf_val = (
        np.r_[: 1.0 : 1j * num_batch * num_node * horizon * 8]
        .reshape(num_batch, num_node, horizon, 8)
        .astype("f")
        * 4.0
    )
    cx.buf[:] = buf_val
...
MPRTheta = collections.namedtuple(
    typename="MPRTheta", field_names="tau I Delta J eta cr".split(" ")
)

mpr_default_theta = MPRTheta(tau=1.0, I=0.0, Delta=1.0, J=15.0, eta=-5.0, cr=1.0)
...
def run_sim_np(
    ...
    def heun(x, cx):
        z = (
            np.random.randn(2, num_node, num_item) * z_scale.reshape((2, 1, 1))
...
def test_step_mpr():
    ...
    weights, lengths, spw_j = rand_weights(
        seed=46, sparsity=sparsity, num_node=num_node, horizon=horizon, dt=dt, cv=cv
    )
    ...
    buf_init_np = buf_val.transpose(1, 2, 0, 3).reshape(
        num_node, horizon, num_batch * 8
    )
    ...
    p[:] = (
        p
        + np.array(mpr_default_theta).reshape(1, 1, -1, 1)
        + np.random.randn(*p.shape) * 0.1
    )
    # set uniform I & cr
    p[..., 1, :] += 2.0
    p[..., 5, :] /= num_node
    ...
    trace_np = trace_np.reshape(-1, num_svar, num_node, num_batch, 8).transpose(
        0, 3, 1, 2, 4
    )

    trace_c = np.zeros_like(
        trace_np
...
def test_perf_step_mpr_np(benchmark):
    ...
    def run1():
        trace_np = run_sim_np(
            ...
            idelays,
            MPRTheta(
                *p.transpose(2, 1, 0, 3).reshape(num_parm, num_node, num_batch * 8)
            ),
            # mpr_default_theta,
            horizon,
            num_item=num_batch * 8,
...
def test_perf_step_mpr_cpp(benchmark):
    ...
    weights, lengths, spw_j = rand_weights(
        seed=42, sparsity=sparsity, num_node=num_node, horizon=horizon, dt=dt, cv=cv
    )
    s_w = scipy.sparse.csr_matrix(weights)
    idelays = (lengths[weights != 0] / cv / dt).astype(np.uint32) + 2
    # idelays = idelays//25 + 2
